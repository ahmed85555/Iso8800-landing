...
export default function App() {
  const [page, setPage] = React.useState('fusa') // 'fusa' | 'sotif' | 'iso8800'
  React.useEffect(() => { window.scrollTo(0, 0) }, [page])

  const Tab = ({ id, label }) => (
    <button
      onClick={() => setPage(id)}
      className={`px-3 py-1.5 rounded-lg text-sm transition ${page === id ? 'bg-gray-900 text-white' : 'hover:bg-gray-100'}`}
    >{label}</button>
  )
    ...
    <div className="min-h-screen w-full text-gray-900 bg-gradient-to-b from-sky-50 via-white to-white">
      ...
      <header className="sticky top-0 z-10 bg-white/70 backdrop-blur border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href="#top" className="font-bold tracking-tight">Ejad • Automotive Safety</a>
      ...
      <section id="top" className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 py-14 md:py-20">
          <motion.h1 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            ...
            {page === 'fusa'   && <>Ejad — End-to-End <span className="bg-gradient-to-r from-sky-600 to-emerald-600 bg-clip-text text-transparent">Functional Safety</span> Services</>}
            {page === 'sotif'  && <>Ejad — <span className="bg-gradient-to-r from-sky-600 to-emerald-600 bg-clip-text text-transparent">SOTIF</span> (ISO 21448) Services</>}
            {page === 'iso8800'&& <>Ejad — Operationalize <span className="bg-gradient-to-r from-sky-600 to-emerald-600 bg-clip-text text-transparent">ISO 8800</span> for AI</>}
          </motion.h1>
          ...
          <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.6 }}
            ...
            {page === 'fusa'    && 'We deliver ISO 26262 across concept, system, hardware/software, and production—integrated with your development V-model and toolchain.'}
            {page === 'sotif'   && 'We engineer Safety of the Intended Functionality: scenario catalogs, insufficiency analysis, measurable coverage and validation for ADAS/DMS.'}
            {page === 'iso8800' && 'We turn ISO/PAS 8800 into a concrete, auditable AI safety pipeline that ties into ISO 26262 and SOTIF.'}
          </motion.p>

          <div className="mt-6 flex flex-wrap gap-2">
            {page === 'fusa' && (<><Badge>HARA / SGs / TSR/TSC</Badge><Badge>ASIL decomposition</Badge><Badge>SW Safety & tool confidence</Badge></>)}
            ...
            <PageShell>
              <div className="grid md:grid-cols-3 gap-4">
                <Card title="Concept & System">
                  Item definition, HARA, safety goals, FTTI, technical safety concept, ASIL tailoring.
                </Card>
                <Card title="HW/SW Development">
                  SW safety requirements, architecture, unit design & verification, tool confidence.
                </Card>
                <Card title="Verification & Production">
                  System test, safety case, production/operation planning, audits.
                </Card>
              </div>
            </PageShell>
          )}
            ...
            <PageShell>
              <div className="grid md:grid-cols-3 gap-4">
                <Card title="Scenario Engineering">
                  ODD-aligned catalogs, boundary/long-tail bins, measurable KPIs.
                </Card>
                <Card title="Insufficient Performance">
                  Hazardous mis-behavior due to perception/planning limits; mitigations.
                </Card>
                <Card title="Validation & Evidence">
                  Sim/replay/track, statistics, confidence bounds, safety argument inputs.
                </Card>
              </div>
            </PageShell>
          )}
            ...
            <PageShell>
              <section id="scope" className="py-8">
                ...
                <p className="text-gray-600 mt-2 max-w-3xl">
                  Per ISO 8800, the AI system is derived from the item. Identify AI components (model vs non-model)
                  before data/model work begins.
                </p>
              </section>
        ...
        <section id="services" className="py-10">
          ...
          <div className="grid md:grid-cols-3 gap-4 mt-6">
            <Card title="Gap Assessment">
              Map your current practice to 26262 / 21448 / 8800. Action plan with prioritized gates and artifacts.
            </Card>
      ...
      <footer className="py-10 border-t border-gray-200 text-sm">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex gap-4">
            <button onClick={() => setPage('fusa')} className="hover:underline">FuSa</button>
            <button onClick={() => setPage('sotif')} className="hover:underline">SOTIF</button>
            <button onClick={() => setPage('iso8800')} className="hover:underline">ISO 8800</button>
          </div>
        </div>
      </footer>
    </div>
